...
- **Soft limiter, tunable from Home Assistant** — opt-in peak limiter with envelope follower that prevents the gain stage from hard-clipping loud transients. When the post-gain signal exceeds the threshold, a proportional gain reduction is applied smoothly and released gradually — so sustained loud passages stay near the ceiling without flat-top distortion. Disabled by default; threshold (−3 dBFS default), attack (5 ms default), and release time (100 ms default) are all tunable from Home Assistant and persisted across reboots. A companion `limiter_gain_reduction_db` diagnostic sensor reports the maximum gain reduction applied in each 5 s window (0 = inactive, positive = active), so you can see at a glance whether the limiter is engaged and how hard it is working. See [docs/configuration.md#soft-limiter](docs/configuration.md#soft-limiter).

Diagnostics and metrics:
- **Diagnostic sensors for Home Assistant** — opt-in `binary_sensor` / `text_sensor` / `sensor` platforms expose client-connected state, client IP, and stream throughput in kbit/s, so you can debug the stream from HA without tailing logs. See [docs/configuration.md#diagnostic-sensors](docs/configuration.md#diagnostic-sensors).
